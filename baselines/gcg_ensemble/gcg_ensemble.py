...
class EnsembleGCG(RedTeamingMethod):
    ...
    def __init__(self,
        ...
        self.current_loss = 'ce-ft' if loss_func_type == 'interleaved' else loss_func_type
        self.loss_threshold = loss_threshold
        self.ft_loss_threshold = ft_loss_threshold
    ...
    def generate_test_cases(self, behaviors, verbose=False, save_dir=None):
        """
        ...
        steps_since_last_updated_stage = 0
        ...
        for i in tqdm(range(self.num_steps)):
            ...
            for worker in gcg_models:
                futures.append(worker.get_token_gradient.remote(optim_ids, num_optim_tokens, current_behavior_indices,
                                                                loss_func_type=self.current_loss))
            ...
            sampled_top_indices_text = self.sample_tokenizer.batch_decode(sampled_top_indices, clean_up_tokenization_spaces=False)
            ...
            optim_str = self.sample_tokenizer.decode(optim_ids[0], clean_up_tokenization_spaces=False)
            ...
            if verbose and ((i % self.eval_steps == 0) or (i == self.num_steps - 1)):
                ...
                if save_dir is not None:
                    test_cases_save_path = self.get_output_file_path(save_dir, None, 'test_cases')
                    ...
                    logs_save_path = self.get_output_file_path(save_dir, None, 'logs')
                    with open(logs_save_path, 'w', encoding='utf-8') as f:
                        json.dump(logs, f, indent=4)
            
            steps_since_last_updated_stage += 1
            if self.loss_func_type == 'interleaved':
                threshold = self.ft_loss_threshold if self.current_loss == 'ce-ft' else self.loss_threshold
                #print(f'\nCurrent Threshold {self.current_loss}: {threshold}\n')
                if all(behavior_losses[:, selected_id] < threshold):
                    self.current_loss = 'ce' if self.current_loss == 'ce-ft' else 'ce-ft'
                    print(f'\nSwitching to {self.current_loss}\n')
                    steps_since_last_updated_stage = 0
                elif (steps_since_last_updated_stage == 20 and self.current_loss == 'ce-ft') or \
                    (steps_since_last_updated_stage == 30 and self.current_loss == 'ce'):
                    self.current_loss = 'ce' if self.current_loss == 'ce-ft' else 'ce-ft'
                    print(f'\nSwitching to {self.current_loss}\n')
                    steps_since_last_updated_stage = 0
            ...
            if self.progressive_behaviors:
                if self.loss_func_type == 'ce' and all(behavior_losses[:, selected_id] < self.loss_threshold):
                    print('\nAdding behavior to list of behaviors to optimize (case 1)\n')
                    num_behaviors += 1
                    steps_since_last_added_behavior = 0
                    print(f'\nNumber Behaviors {num_behaviors}\n')
                    if num_behaviors > len(all_behavior_indices):
                        break
                elif self.loss_func_type == 'ce-ft' and all(behavior_losses[:, selected_id] < self.ft_loss_threshold):
                    print('\nAdding behavior to list of behaviors to optimize (case 1)\n')
                    num_behaviors += 1
                    print(f'\nNumber Behaviors {num_behaviors}\n')
                    steps_since_last_added_behavior = 0
                    if num_behaviors > len(all_behavior_indices):
                        break
                elif self.loss_func_type == 'interleaved':                    
                    another_loss_type = 'ce' if self.current_loss == 'ce-ft' else 'ce-ft'
                    _futures = []
                    for _worker in gcg_models:
                        _future = _worker.compute_loss_on_candidates_batch.remote(sampled_top_indices, current_behavior_indices,
                                                                                loss_func_type=another_loss_type)
                        _futures.append(_future)
                    _losses = ray.get(_futures)
                    _losses = torch.stack(_losses)
                    another_behavior_losses = _losses.mean(0)
                    if self.current_loss == 'ce-ft':
                        ce_loss, ceft_loss = another_behavior_losses, behavior_losses
                    else:
                        ce_loss, ceft_loss = behavior_losses, another_behavior_losses
                    if all(ceft_loss[:, selected_id] < self.ft_loss_threshold) and all(ce_loss[:, selected_id] < self.loss_threshold):
                        print('\nAdding behavior to list of behaviors to optimize (case 1)\n')
                        num_behaviors += 1
                        print(f'\nNumber Behaviors {num_behaviors}\n')
                        steps_since_last_added_behavior = 0
        ...
        logs = {'final_loss': current_loss, 'all_losses': all_losses, 'all_test_cases': all_test_cases, 'num_behaviors': num_behaviors, 'suffix': optim_str}
    ...
    def save_test_cases(self, save_dir, test_cases, logs=None, method_config=None, run_id=None):
        ...
        suffix_save_path = os.path.join(save_dir, f"suffix_{run_id}.json")
        ...
        if logs is not None:
            ...
            with open(suffix_save_path, 'w', encoding='utf-8') as f:
                json.dump({'suffix': logs['suffix']}, f, indent=4)
